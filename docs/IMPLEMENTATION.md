...
- 외부 CDN을 런타임에 부르지 않는다. 예외는 PowerPoint 애드인 페이지의 `office.js` 하나다. 애드인은 이 스크립트를 번들에 넣을 수 없고 없으면 실행되지 않는다. 애드인 페이지의 CSP만 그 출처를 허용하고, 대신 애드인 호스트는 토큰을 브라우저 저장소에 쓰지 않고 메모리에만 둔다 (같은 출처의 웹앱 저장소를 그 스크립트가 읽을 수 있기 때문).
