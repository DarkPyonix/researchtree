...
4. **읽기 편한 것이 기본, 아름다운 것은 모드로.** 실험 트리는 라벨과 수치를 읽는 것이 핵심이다. 2D 트리를 기본 화면으로 하고, 3D의 "자라나는 트리"는 데모/발표 모드로 둔다.
5. **기본은 설치 없이.** URL 하나만 열면 트리가 보여야 한다. 교수님이나 다른 연구실에도 링크 하나로 공유할 수 있어야 한다. VS Code 확장과 로컬 실행은 같은 뷰어를 다른 곳에서 여는 추가 경로다.
6. **연구 브랜치만 보여준다.** 레포에는 `main`, `develop` 같은 연구 외 브랜치가 함께 있다. 트리는 `research`와 `experiment/*`만 다룬다.
...
- 뷰어는 **TypeScript로 만든 SPA 하나**다. Vite로 빌드하고, 2D는 d3, 3D는 three.js를 쓴다.
...
- PyPI 패키지 `researchtree`는 두 가지를 제공한다.
