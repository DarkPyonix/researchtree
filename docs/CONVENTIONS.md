...
글을 세 종류로 나눈다. 섞이면 문서가 계속 커져서 아무도 읽지 않게 되고, 실험 결과와 문서가 조용히 어긋난다.
...
- 경로는 `research` 브랜치 루트의 `.researchtree.yml`로 바꾼다(`spec`, `intent`). 같은 파일의 `prefix`는 실험 브랜치 접두사를 팀 전체에 정한다. 뷰어의 개인 설정보다 이 파일이 먼저이고, Python에서는 인자와 환경변수가 이 파일보다 먼저다. 루트 브랜치 이름은 적을 수 없다(파일이 그 브랜치 안에 있다).
