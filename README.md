...
<table>
  ...
  <tr><td>🗺️ <b>Flat view</b></td><td>Press <b>Flat view</b> and the camera turns top-down while the plants are pressed flat. Ground, water, landings, ferries and decorations disappear, leaving the dirt paths, plots, version stones and label chips on the page; sea crossings become dashed path tiles so branches stay connected. In flat view you pan and zoom only. The same button then cycles on: <b>Tree view</b> turns the flat view so the first version is at the bottom and the latest at the top, <b>3D view</b> raises the islands at that angle, and <b>Island view</b> goes back to the first view. Your selection is kept throughout.</td></tr>
